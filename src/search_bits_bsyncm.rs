...
//bounded syncmers
fn sliding_window_bsyncmers_skip_n_u64(seq: &str, k: usize, m: usize) -> Vec<u64> {
    let mut vec: Vec<u64> = Vec::new();
    let mut window: VecDeque<(u64, usize, u64)> = VecDeque::new(); //minimizer, position,  non-canonical minimizer
    let length = seq.len();
    let window_size = 2 * k - m - 1;
    let mut sequence_u64: Vec<u64> = Vec::new();
    let mut counter = 0;
    let mut i = 0;
    ...
    for n in seq.bytes() {
        ...
        if new_char < 4 {
            ...
            if counter >= m {
                ...
                if i >= window_size {
                    let best_index = window.front().unwrap().1;
                    let mut best_non_canonical = window.front().unwrap().2;
                    if best_index + k - 1 < i {
                        for u in &sequence_u64[best_index + m..best_index + k] {
                            best_non_canonical <<= 2;
                            best_non_canonical |= u;
                        }
                        best_non_canonical &= k_mask;
                        vec.push(kmer::canonical(best_non_canonical, k)); //has to be canonical in production
                    } else {
                        for u in &sequence_u64[best_index - (k - m)..best_index] {
                            kmer_u64 <<= 2;
                            kmer_u64 |= u;
                        }
                        kmer_u64 <<= m * 2;
                        kmer_u64 |= best_non_canonical;
                        kmer_u64 &= k_mask;
                        vec.push(kmer::canonical(kmer_u64, k));
                    }
                }
            }
            i += 1;
